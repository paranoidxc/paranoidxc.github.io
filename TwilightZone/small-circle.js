...
class SmallCircle {
  ...
  update() {
    let distancePositionToMouse = dist(
      mouseX,
      mouseY,
      this.position.x,
      this.position.y
    );
    let distancePositionToBigCircle = dist(
      this.position.x,
      this.position.y,
      width / 2,
      width / 2
    );
    let distancePositionToCorner = dist(
      this.position.x,
      this.position.y,
      POSITION_CORNER.x,
      POSITION_CORNER.y
    ...
    switch (this.state) {
      ...
      case SmallCircleState.MOVING_TO_INSIDE: {
        ...
        this.position.x +=
          this.EASING_FACTOR * (this.targetPosition.x - this.position.x);
        this.position.y +=
          this.EASING_FACTOR * (this.targetPosition.y - this.position.y);
