...
function setup() {
  ...
  POSITION_CORNER = createVector(
    CANVAS_SIZE / 2,
    CANVAS_SIZE / 2 + 250 * FACTOR
  );
  p1 = new SmallCircle(
    POSITION_CENTER.x,
    POSITION_CENTER.y,
...
function draw() {
  ...
  if (
    p1.state == SmallCircleState.DRAGGING_FROM_INSIDE ||
    p1.state == SmallCircleState.DRAGGING_FROM_OUTSIDE
  ) {
    if (
      dist(p1.position.x, p1.position.y, POSITION_CENTER.x, POSITION_CENTER.y) <
      BIG_CIRCLE_RADIUS / 2
    ) {
  ...
  if (
    p2.state == SmallCircleState.DRAGGING_FROM_INSIDE ||
    p2.state == SmallCircleState.DRAGGING_FROM_OUTSIDE
  ) {
    if (
      dist(p2.position.x, p2.position.y, POSITION_CENTER.x, POSITION_CENTER.y) <
      BIG_CIRCLE_RADIUS / 2
    ) {
